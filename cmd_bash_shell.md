...
~~~bash
  ...
  ! root:密码MD5:创建日期值[1970-1-1为第1天]:不能被修改的天数[0为不限制]:强制需修改的天数[99999为不限制]:提醒需修改的天数起日[7表示第7天起提醒强制需修改密码]:宽限登录日期[7表示强制需修改密码日期过后7天内为宽限期\过期后登录账号时强制改密码]:有效期限[用于商业购买年月]:其它用途😊
  $ cat /etc/group  # 用户组列表
  $ groups          # 用户所在组
  $ groupadd        # 添加用户组
  $ passwd admin    # 修改密码
