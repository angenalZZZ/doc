...
* [安装*OpenJDK 11 On redhat](https://developers.redhat.com/products/openjdk/download)
* [安装*WIX Toolset创建*Windows Installer](https://wixtoolset.org/)
...
$Path = "C:\ProgramData\Your-App"
$User = "ANGENAL-PC\Administrator"
$Acl = Get-Acl $Path
$A = New-Object System.Security.AccessControl.FileSystemAccessRule($User,"Modify","ContainerInherit,ObjectInherit","None","Allow")
$Acl.SetAccessRule($A)
Set-Acl $Path $Acl
~~~
> Windows 10 系统问题排查
...
配置：
   1、连接方式 选择 桥接网卡
   2、界面名称 选择 （如果有无线网卡和有线网卡，需要根据现在的上网方式对应选择）
   3、高级-控制芯片 Intel PRO/*
   4、高级-混杂模式 拒绝
   5、高级-接入网线 √（虚拟机ip自动获取）
IP样式：
...
配置：
   ...
   4、高级-混杂模式 拒绝
   5、高级-接入网线 √（虚拟机ip自动获取，也可以手动配置，网关默认配置192.168.56.1）
IP样式：
...
配置：
   ...
   3、高级-控制芯片 PCnet-FAST*
   4、高级-混杂模式 拒绝
   5、高级-接入网线 √（虚拟机ip自动获取，但linux必须手动配置并保证各个虚拟机ip在同一网段）
ip样式：
   ip 169.254.147.9
   子网掩码 255.255.0.0
   网关 无
...
> [开启WSA - Windows Subsystem for Android](https://appuals.com/this-app-will-not-work-on-your-device-error-windows-11)、`推荐`[`Android`模拟器`Nox`](https://shareappscrack.com/noxplayer-nox-app-player/)[`夜神`](https://www.yeshen.com/)、[`Android`模拟器`LD雷电`](https://www.ldplayer.net/)、[BlueStacks`模拟器`](https://www.bluestacks.com/download.html)<br>
...
../configure                                # 默认  --prefix=/usr/local/  --with-binutils=/usr/local/bin
make                                        # 编译
make install                                # 安装
