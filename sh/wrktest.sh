...
: << EOF
API 性能测试脚本，会自动执行 wrk 命令，采集数据、分析数据，最后调用 gnuplot 生成图表

使用方式 (测试 API 性能)
1. 启动程序： ./apiserver (8080端口)
2. 执行命令： ./wrktest.sh apiserver 8080
...
使用方式 (对比2次测试结果)
 执行命令： ./wrktest.sh diff apiserver_wrk.dat http_wrk.dat

> 注意: 需要确保系统安装了 wrk 和 gnuplot 工具
EOF

# sh 输入参数
job=${1:-apiserver} # 本次测试名称
port=${2:-8080} # http端口号

t1=${1:-apiserver} # 对比图中红色线条名称
t2="http" # 对比图中粉色线条名称
...
apiperformance="${job}_performance.png"
apisuccessrate="${job}_success_rate.png"
datfile="${job}_wrk.dat"
